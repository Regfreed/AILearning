...
def getTwoPeaks(binary_img):
    x_array = range(0,binary_img.shape[1])
...
def slidingWindow(binary_img, color_img, x_left, x_right):
    ...
    # ako nema piksela koristi se stare vrijednosti pozicije prozora
    x_last = x_pos
    y_last = y_pos
    ...
    return color_img, leftLine, rightLine
...
def plotPolyLine(warped_img, leftLine, rightLine):
    ...
    try:
        for xl, xr, y in zip(xls, xrs, ys):
            cv2.line(warped_img, (xl - 4, y), (xl + 4, y), (255, 255, 0), int(t / 2))
            cv2.line(warped_img, (xr - 4, y), (xr + 4, y), (0, 0, 255), int(t / 2))
    except:
        print("some error")   
    ...
    return warped_img, left_xs, right_xs, ys
...
while(True):
    ...
    # izracunaj pocetne vrijednosti x koordinate za pomicne prozore
    x_left, x_right = getTwoPeaks(mask)
    
    # nacrtaj prozore, spremi piksele lijeve i desne oznake u numpy polja
    windowPlot, leftLine, rightLine = slidingWindow(mask, frameFiltered, x_left, x_right)

    # prikazi linije
    windowPlot, left_xs, right_xs, ys = plotPolyLine(windowPlot, leftLine, rightLine)

    # prikazi voznu traku
    frameLane = showLane(frame, left_xs, right_xs, ys, M_inv)
